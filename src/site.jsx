...
function DecisionsDependenciesPanel({ deliverable, stepDeps, onward, idMap }) {
  ...
  return <section className="panel decision-dependency-panel" id="decisions-dependencies"><h2>Cross-cutting decisions and dependencies</h2><p className="subtle cross-cutting-scope-note">Only whole-deliverable decisions, cross-deliverable dependencies and escalation points are shown here. Step-level sequencing, handoffs and operational choices sit in the Delivery timeline.</p><div className="decision-dependency-grid"><div><h3>Whole-deliverable decisions</h3>{decisions.length ? <div className="decision-card-stack">{decisions.map((item, index) => <article className="schema-card" key={index}><h4>{asText(item)}</h4>{desc(item) && <p>{desc(item)}</p>}<p className="schema-meta">{meta([item.owner, item.decisionMaker, item.decisionNeededBy || item.neededBy, item.confidence])}</p></article>)}</div> : <p>No whole-deliverable decisions or escalation points captured yet.</p>}</div><div><h3>Cross-deliverable dependencies</h3>{stepDeps.length ? <><p className="subtle">Cross-deliverable depends on</p><div className="link-list">{stepDeps.map((id) => <SmartLink key={id} id={id} idMap={idMap} />)}</div></> : <p>No cross-deliverable dependencies or handoffs captured yet. Step-level sequencing is shown in the Delivery timeline.</p>}{onward.length ? <><p className="subtle">Cross-deliverable feeds into</p><ul className="compact-list">{onward.map((entry, index) => <li key={`${entry.parent.id}-${entry.step.id}-${index}`}><SmartLink id={entry.step.id} idMap={idMap} /></li>)}</ul></> : <p>No onward cross-deliverable handoffs captured yet.</p>}</div></div></section>;
}
...
function TimelineView({ timelineItems, idMap, dependencyIndex }) {
  ...
  const periodBuckets = periods.reduce((buckets, period) => {
    const current = buckets[buckets.length - 1];
    if (current?.id === period.bucketId) current.periods.push(period);
    else buckets.push({ id: period.bucketId, label: period.bucketLabel, periods: [period] });
    return buckets;
  }, []);
  const [projectFilter, setProjectFilter] = useState('all');
  const [query, setQuery] = useState('');
  ...
  const filteredByProject = eligible.filter((item) => projectFilter === 'all' || item.project?.id === projectFilter);
  const rows = filteredByProject.filter((item) => {
    const haystack = [item.id, displayId(item), item.title, item.summary, item.ownerLabel, item.project?.title, item.project?.id, ...item.steps.flatMap((step) => [step.id, step.title, step.summary, periodLabel(step.period)])].filter(Boolean).join(' ').toLowerCase();
    return haystack.includes(query.trim().toLowerCase());
  });
  const visibleSteps = rows.flatMap((item) => item.steps || []);
  const visibleDependencyCount = visibleSteps.reduce((total, step) => total + getStepDependencies(step).length, 0);
  const selectedDependencyLabels = selectedDeps.map((id) => resolveLabel(id, idMap));
  const gridTemplate = { gridTemplateColumns: `320px repeat(${periods.length}, minmax(92px, 1fr))` };
  const laneTemplate = { gridTemplateColumns: `repeat(${periods.length}, minmax(92px, 1fr))` };
  ...
  return <main className="timeline-page"><section className="section-heading timeline-heading"><div><h1>Timeline</h1><p>Scan delivery sequence by period, search for a step or deliverable, and click any step to reveal immediate dependencies and downstream handoffs. Only deliverables that have moved beyond pre-draft are shown.</p></div><div className="timeline-summary-cards"><article><strong>{rows.length}</strong><span>deliverables</span></article><article><strong>{visibleSteps.length}</strong><span>steps</span></article><article><strong>{visibleDependencyCount}</strong><span>dependency links</span></article></div></section><div className="timeline-controls"><label><span>Search timeline</span><input type="search" placeholder="Find step, owner, project or period" value={query} onChange={(event) => setQuery(event.target.value)} /></label><label><span>Project</span><select value={projectFilter} onChange={(event) => setProjectFilter(event.target.value)}><option value="all">All projects</option>{projectOptions.map((project) => <option key={project.id} value={project.id}>{displayId(project)} {project.title}</option>)}</select></label>{selectedStepId && <button type="button" className="secondary-button" onClick={() => setSelectedStepId(null)}>Clear selection</button>}</div><section className={`timeline-inspector ${selectedStep ? 'active' : ''}`} aria-live="polite">{selectedStep ? <><div><p className="eyebrow">Selected step</p><h2>{selectedStep.title}</h2><p>{selectedParent ? `${displayId(selectedParent)} ${selectedParent.title}` : 'Timeline step'} · {periodLabel(selectedStep.period)}</p></div><div><h3>Depends on</h3>{selectedDependencyLabels.length ? <ul>{selectedDependencyLabels.map((label) => <li key={label}>{label}</li>)}</ul> : <p>No dependencies captured.</p>}</div><div><h3>Feeds into</h3>{selectedOnward.length ? <ul>{selectedOnward.map((entry, index) => <li key={`${entry.parent.id}-${entry.step.id}-${index}`}>{displayId(entry.parent)}: {entry.step.title}</li>)}</ul> : <p>No onward handoffs captured.</p>}</div><button type="button" className="secondary-button" onClick={() => setModalStepId(selectedStepId)}>Open detail</button></> : <><div><p className="eyebrow">How to use</p><h2>Pick a step to trace the path</h2><p>Selected steps turn black, prerequisites turn red, and downstream handoffs use a red outline.</p></div></>}</section><div className="timeline-key"><span><i className="key-box selected-key" /> Selected</span><span><i className="key-box dependency-key" /> Depends on</span><span><i className="key-box dependent-key" /> Feeds into</span><span><i className="dependency-dot">↳</i> Has dependencies</span></div>{rows.length === 0 ? <EmptyState title="No timeline items to show yet"><p>{eligible.length === 0 ? 'Timeline items will appear here once deliverables have moved beyond pre-draft.' : 'No timeline items match the current filters.'}</p></EmptyState> : <div className="timeline timeline-refresh" aria-label="Delivery timeline"><div className="timeline-bucket-header timeline-grid" style={gridTemplate}><div>Planning horizon</div>{periodBuckets.map((bucket) => <div key={bucket.id} style={{ gridColumn: `span ${bucket.periods.length}` }}>{bucket.label}</div>)}</div><div className="timeline-header timeline-grid" style={gridTemplate}><div>Project / deliverable</div>{periods.map((period) => <div key={period.id}>{period.shortLabel}</div>)}</div>{rows.map((item) => <div className={`timeline-row timeline-grid ${item.project.deliveryContext === 'out-of-programme' ? 'enabling-row' : ''}`} style={gridTemplate} key={item.id}><a className="timeline-title" href={`#/deliverables/${item.id}`}><DeliverableContextLine deliverable={item} /><strong>{item.title}</strong><span>{item.ownerLabel}</span><em>{item.steps.length} steps · {item.steps.reduce((total, step) => total + getStepDependencies(step).length, 0)} links</em></a><div className="timeline-lane" style={laneTemplate}>{item.steps.map((step) => <button type="button" className={blockClass(item, step)} style={stepStyle(step)} key={step.id} title={`${periodLabel(step.period)}. Click to inspect; double-click for details.`} onClick={() => setSelectedStepId(step.id)} onDoubleClick={() => { setSelectedStepId(step.id); setModalStepId(step.id); }}><span>{step.title}</span><small>{periodLabel(step.period)}</small>{getStepDependencies(step).length > 0 && <span className="dependency-dot">↳</span>}</button>)}</div></div>)}</div>}<TimelineStepModal step={modalStep} parent={modalParent} deps={modalDeps} onward={modalOnward} idMap={idMap} onClose={() => setModalStepId(null)} /></main>;
}
