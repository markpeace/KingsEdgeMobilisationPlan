...
function packageClass(deliverable) {
  const link = deliverable.workPackageLinks?.[0];
  return link ? `package-linked package-${link.position}` : '';
}
...
function WorkPackageBadges({ deliverable }) {
  if (!hasItems(deliverable.workPackageLinks)) return null;
  return <div className="package-badges">{deliverable.workPackageLinks.map((link) => <span className="package-badge" title={link.summary} key={link.id}>{link.title}</span>)}</div>;
}
...
function ProjectsPage() {
  return <main><section className="section-heading projects-heading"><h1>Projects View</h1><p>Projects are shown in their working order. Linked deliverables are marked as work packages where activity cuts across columns.</p></section><div className="project-board"><div className="project-scroll">{edgeProjects.map((project) => <ProjectColumn key={project.id} project={project} />)}<div className="programme-divider" aria-label="Scroll right for out of programme projects"><span>→ → →</span></div>{outOfProgramme.map((project) => <ProjectColumn key={project.id} project={project} />)}</div></div></main>;
}

function ProjectColumn({ project }) {
  const isOut = project.deliveryContext === 'out-of-programme';
  return <section className={`project-column ${isOut ? 'related-project-column' : ''}`}><a className={`project-column-header project-header-link ${isOut ? 'related-project-header' : ''}`} href={`#/projects/${project.id}`}><span className="reference">{isOut ? 'Out' : displayId(project)}</span><h2>{project.title}</h2><p>{project.summary}</p><p className="owner">Owner: {project.owner}</p></a><div className="deliverable-stack">{project.deliverables.map((deliverable) => <a className={`deliverable-card ${packageClass(deliverable)}`} href={`#/deliverables/${deliverable.id}`} key={deliverable.id}><span className="reference">{displayId(deliverable)}</span><h3>{deliverable.title}</h3><p>{deliverable.summary}</p><WorkPackageBadges deliverable={deliverable} /><StatusPills id={deliverable.id} compact /><p className="lead">Lead: {deliverable.lead}</p></a>)}</div></section>;
}

function ProjectDeliverableColumn({ deliverable }) {
  return <section className={`project-deliverable-column ${packageClass(deliverable)}`}><a className="project-deliverable-header" href={`#/deliverables/${deliverable.id}`}><span className="reference">{displayId(deliverable)}</span><h3>{deliverable.title}</h3><p>{deliverable.summary}</p><WorkPackageBadges deliverable={deliverable} /><StatusPills id={deliverable.id} compact /><p className="lead">Lead: {deliverable.lead}</p></a><div className="project-step-stack">{deliverable.steps.map((step) => <ProjectStepCard key={step.id} step={step} deliverable={deliverable} />)}</div></section>;
}
...
function TimelineView({ timelineItems, idMap, dependencyIndex }) {
  ...
  return <main><section className="section-heading"><h1>Timeline</h1><p>Click a step to show the interplay between delivery steps across Edge and out of programme projects.</p></section><div className="toolbar timeline-toolbar"><select value={projectFilter} onChange={(event) => setProjectFilter(event.target.value)}><option value="all">All projects</option>{projects.map((project) => <option key={project.id} value={project.id}>{displayId(project)} {project.title}</option>)}<option value="related">Out of programme projects</option></select><label className="toggle-label"><input type="checkbox" checked={showRelated} onChange={(event) => setShowRelated(event.target.checked)} /> Show out of programme projects</label>{selectedStepId && <button type="button" className="secondary-button" onClick={() => setSelectedStepId(null)}>Clear selection</button>}</div><DependencyLens step={selectedStep} parent={selectedParent} deps={selectedDeps} onward={selectedOnward} idMap={idMap} /><div className="timeline-key"><span><i className="key-box selected-key" /> Selected</span><span><i className="key-box dependency-key" /> Depends on</span><span><i className="key-box dependent-key" /> Feeds into</span><span><i className="key-box status-active-key" /> Active</span><span><i className="key-box status-scoping-key" /> Scoping</span></div><div className="timeline"><div className="timeline-header"><div>Project / deliverable</div>{periods.map((period) => <div key={period.id}>{period.shortLabel}</div>)}</div>{rows.map((item) => <div className={`timeline-row ${item.project.deliveryContext === 'out-of-programme' ? 'enabling-row' : ''}`} key={item.id}><a className="timeline-title" href={`#/deliverables/${item.id}`}><span className="reference">{displayId(item)}</span><strong>{item.title}</strong><span>{item.ownerLabel}</span><WorkPackageBadges deliverable={item} /><StatusPills id={item.id} compact /></a>{periods.map((period) => <div className="timeline-cell" key={`${item.id}-${period.id}`}>{item.steps.filter((step) => step.period === period.id).map((step) => <button type="button" className={blockClass(step)} key={step.id} onClick={() => setSelectedStepId(step.id)}><span>{step.title}</span>{getStepDependencies(step).length > 0 && <span className="dependency-dot">↳</span>}</button>)}</div>)}</div>)}</div></main>;
}
