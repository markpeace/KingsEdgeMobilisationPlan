...
function SmartLink({ id, idMap }) {
  ...
  if (result.type === 'project') return <a className="chip" href={`#/projects/${id}`}>{result.item.title}</a>;
...
function ProjectDetail({ project }) {
  ...
  const isOut = project.deliveryContext === 'out-of-programme';
  return <main><a className="back-link" href="#/projects">Back to Projects View</a><section className={`detail-hero project-detail-hero ${isOut ? 'related-project-detail-hero' : ''}`}><h1>{project.title}</h1><p>{project.summary}</p>{project.edgeRole && <p>{project.edgeRole}</p>}<div className="detail-meta"><span>Project owner: {project.owner}</span><span>{project.deliverables.length} deliverables</span><span>{isOut ? 'Out of programme' : 'Edge programme'}</span></div></section><section className="panel"><h2>Deliverables</h2><div className="component-grid project-detail-deliverables">{project.deliverables.map((deliverable) => <a className="component-card project-detail-card" href={`#/deliverables/${deliverable.id}`} key={deliverable.id}><span className="reference">{deliverable.id}</span><h3>{deliverable.title}</h3><p>{deliverable.summary}</p><StatusPills id={deliverable.id} compact /><p className="lead">Lead: {deliverable.lead}</p></a>)}</div></section>{project.servesDeliverables && <section className="panel"><h2>Serves Edge deliverables</h2><div className="link-list">{project.servesDeliverables.map((id) => <a className="chip" href={`#/deliverables/${id}`} key={id}>{id}</a>)}</div></section>}</main>;
}
