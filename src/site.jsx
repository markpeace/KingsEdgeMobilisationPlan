...
function Landing() {
  return <main className="landing-main"><section className="hero landing-hero"><p className="eyebrow">Interactive delivery map</p><h1>{plan.programme.title}</h1><p>{plan.programme.purpose}</p><div className="landing-links"><a href="#/projects"><span>01</span><strong>Projects view</strong><em>Browse the core projects and the related out of programme work.</em></a><a href="#/deliverables"><span>02</span><strong>Deliverables index</strong><em>Search and filter by project and planning status.</em></a><a href="#/measures"><span>03</span><strong>Measures</strong><em>Track draft and emerging benefit measures across deliverables.</em></a><a href="#/timeline"><span>04</span><strong>Timeline</strong><em>View indicative sequencing and step-to-step dependencies.</em></a></div></section></main>;
}
...
function MeasuresView({ deliverables }) {
  ...
  const visibleMeasures = showEmerging ? measures : measures.filter((entry) => !isPreDraft(entry.deliverable));
  const emergingCount = measures.length - visibleMeasures.length;
  const types = [...new Set(visibleMeasures.map((entry) => entry.measure.measureType).filter(Boolean))].sort();
  const filtered = visibleMeasures.filter((entry) => (projectFilter === 'all' || entry.deliverable.project.id === projectFilter) && (typeFilter === 'all' || entry.measure.measureType === typeFilter));
  const projectsWithMeasures = new Set(visibleMeasures.map((entry) => entry.deliverable.project.id)).size;
  return <main><section className="section-heading"><h1>Measures</h1><p>Track benefit measures and evidence questions. Pre-draft measures are treated as emerging and are hidden by default.</p></section><div className="measure-summary"><article className="measure-card"><span>{visibleMeasures.length}</span><strong>{showEmerging ? 'Measures shown' : 'Draft or later measures'}</strong></article><article className="measure-card"><span>{projectsWithMeasures}</span><strong>Projects represented</strong></article><article className="measure-card"><span>{emergingCount}</span><strong>Emerging hidden</strong></article></div><div className="toolbar"><select value={projectFilter} onChange={(event) => setProjectFilter(event.target.value)}><option value="all">All projects</option>{projects.map((project) => <option key={project.id} value={project.id}>{displayId(project)} {project.title}</option>)}</select><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="all">All measure types</option>{types.map((type) => <option key={type} value={type}>{type}</option>)}</select><label className="toolbar-check"><input type="checkbox" checked={showEmerging} onChange={(event) => setShowEmerging(event.target.checked)} /> Show emerging pre-draft measures</label></div>{filtered.length === 0 ? <section className="panel"><h2>No measures to show yet</h2><p>Pre-draft measures are hidden by default so emerging evidence questions are not presented as approved KPIs.</p></section> : <div className="index-list measure-list">{filtered.map(({ id, deliverable, measure }) => <a href={`#/deliverables/${deliverable.id}`} className={`index-row measure-row ${isPreDraft(deliverable) ? 'emerging-measure-row' : ''}`} key={id}><div><span className="reference">{displayId(deliverable)}</span><PlanningStatusTag item={deliverable} /><h3>{measure.title}</h3><p>{[measure.questionAnswered, measure.measure, measure.target ? `Target: ${measure.target}` : null, measure.baseline ? `Baseline: ${measure.baseline}` : null, measure.dataSource ? `Source: ${measure.dataSource}` : null].filter(Boolean).join(' · ')}</p></div><div className="index-meta"><span>{displayId(deliverable.project)} {deliverable.project.title}</span><strong>{measure.measureType || 'measure'}</strong><span>{isPreDraft(deliverable) ? 'Emerging measure' : measure.confidence || 'Confidence TBC'}</span></div></a>)}</div>}</main>;
}
...
function StaffSummaryPanel({ deliverable }) {
  return <section className="panel staff-summary-panel"><h2>Planning summary</h2><div className="staff-summary-grid"><FieldCard title="Current planning status"><p>{planningStatusLabel(planningStatus(deliverable))}</p>{isPreDraft(deliverable) && <p>This deliverable is included in the mobilisation map, but has not yet completed detailed planning scrutiny.</p>}</FieldCard><FieldCard title="Current planning focus"><p>Clarify the case for change, benefits, outputs, measures, resources and ownership before moving this deliverable to draft.</p></FieldCard><FieldCard title="Next scrutiny step"><p>Review the detailed plan, confirm assumptions and decide whether this deliverable is ready to move from pre-draft to draft.</p></FieldCard></div></section>;
}
...
function TimelineView({ timelineItems, idMap, dependencyIndex }) {
  ...
  const rows = timelineItems.filter((item) => projectFilter === 'all' || item.project?.id === projectFilter);
  ...
  const blockClass = (item, step) => ['timeline-block', statusClass(getStatus(step.id).status), isPreDraft(item) ? 'timeline-block-indicative' : '', selectedStepId === step.id ? 'selected' : '', selectedDeps.includes(step.id) ? 'dependency-highlight' : '', onwardIds.has(step.id) ? 'dependent-highlight' : '', selectedStepId && selectedStepId !== step.id && !selectedDeps.includes(step.id) && !onwardIds.has(step.id) ? 'dimmed' : ''].join(' ');
  const stepStyle = (step) => ({ gridColumn: `${getStepPeriodSpan(step.period).startIndex} / span ${getStepPeriodSpan(step.period).span}` });
  return <main className="timeline-page"><section className="section-heading"><h1>Timeline</h1><p>Click a step to highlight dependencies. Pre-draft timeline items are indicative planning assumptions.</p></section><div className="timeline-controls"><select value={projectFilter} onChange={(event) => setProjectFilter(event.target.value)}><option value="all">All projects</option>{projects.filter((project) => project.deliverables?.length).map((project) => <option key={project.id} value={project.id}>{displayId(project)} {project.title}</option>)}</select>{selectedStepId && <button type="button" className="secondary-button" onClick={() => setSelectedStepId(null)}>Clear selection</button>}</div><div className="timeline-key"><span><i className="key-box selected-key" /> Selected</span><span><i className="key-box dependency-key" /> Depends on</span><span><i className="key-box dependent-key" /> Feeds into</span><span><i className="key-box indicative-key" /> Indicative</span></div><div className="timeline timeline-refresh"><div className="timeline-header timeline-grid" style={gridTemplate}><div>Project / deliverable</div>{periods.map((period) => <div key={period.id}>{period.shortLabel}</div>)}</div>{rows.map((item) => <div className={`timeline-row timeline-grid ${item.project.deliveryContext === 'out-of-programme' ? 'enabling-row' : ''} ${isPreDraft(item) ? 'timeline-row-predraft' : ''}`} style={gridTemplate} key={item.id}><a className="timeline-title" href={`#/deliverables/${item.id}`}><span className="reference">{displayId(item)}</span><PlanningStatusTag item={item} /><strong>{item.title}</strong><span>{item.ownerLabel}</span></a><div className="timeline-lane" style={laneTemplate}>{item.steps.map((step) => <button type="button" className={blockClass(item, step)} style={stepStyle(step)} key={step.id} title={`${periodLabel(step.period)}. ${isPreDraft(item) ? 'Indicative pre-draft planning.' : 'Double-click for details.'}`} onClick={() => setSelectedStepId(step.id)} onDoubleClick={() => { setSelectedStepId(step.id); setModalStepId(step.id); }}><span>{step.title}</span>{isPreDraft(item) && <span className="indicative-label">Indicative</span>}{getStepDependencies(step).length > 0 && <span className="dependency-dot">↳</span>}</button>)}</div></div>)}</div><TimelineStepModal step={modalStep} parent={modalParent} deps={modalDeps} onward={modalOnward} idMap={idMap} onClose={() => setModalStepId(null)} /></main>;
}
