...
import { periodLabel } from './plan-utils.js';
...
function formatPeriodNeeded(value) {
  if (!value) return null;
  return typeof value === 'string' ? value : periodLabel(value);
}
...
function ExistingTable({ asks, conditional = false, unquantified = false }) {
  ...
  return <div className={`resource-table-block ${conditional ? 'is-conditional' : ''}`}><div className="resource-subheading"><div><h4>{title}</h4><p>{description}</p></div></div><div className="resource-readable-table-wrap"><table className="resource-readable-table resource-existing-table"><thead><tr><th scope="col">Role / contribution</th><th scope="col">{conditional ? 'Maximum' : 'Commitment'}</th><th scope="col">When needed</th><th scope="col">Owner</th></tr></thead><tbody>{asks.map((ask) => <tr key={`${ask.sourceStep?.contextId || ''}-${ask.id || askName(ask)}`}><th scope="row">{askName(ask)}</th><td>{typeof ask.fte === 'number' ? formatFte(ask.fte, conditional ? 'Up to ' : '') : 'As required'}</td><td>{formatPeriodNeeded(ask.periodNeeded) || 'TBC'}</td><td>{ask.owner || 'TBC'}</td></tr>)}</tbody></table></div></div>;
}
...
export function ResourceInvestmentProfile({ context }) {
  ...
  const hasStepResourceAsks = profile.asks.length > 0;
  ...
  return <section id="resource-investment-profile" className="panel resource-investment-profile"><header className="resource-profile-titlebar"><h2>{context.type === 'project' ? 'Project resources and investment' : 'Resources and investment'}</h2><p>A decision-ready view of mobilisation spend, appointments, existing institutional capacity and the intended business-as-usual landing.</p></header><div className="resource-profile-body resource-readable-body">{hasStepResourceAsks ? <div className="resource-profile-summary-grid resource-readable-summary">{cards}</div> : null}<WorkforceCallout context={context} />{hasStepResourceAsks ? <><CapacityStrip profile={profile} /><YearStrip profile={profile} context={context} />{mobilisationUnresolved || bauUnresolved ? <p className="resource-attention-line">{mobilisationUnresolved ? <strong>{mobilisationUnresolved} mobilisation {mobilisationUnresolved === 1 ? 'item needs' : 'items need'} approval or validation. </strong> : null}{bauUnresolved ? <span>{bauUnresolved} BAU recurrent {bauUnresolved === 1 ? 'commitment still needs' : 'commitments still need'} its funding route closed.</span> : null}</p> : null}<PeopleSection profile={profile} context={context} /><BauSection profile={profile} context={context} /><OtherInvestmentSection profile={profile} /><PlanningDetail profile={profile} steps={steps} /></> : <p className="subtle resource-profile-source">Shared workforce allocation is recorded at programme level. Detailed step-level resource asks will be added as this deliverable reaches delivery design.</p>}</div></section>;
}
